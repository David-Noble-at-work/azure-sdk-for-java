 ...
 * sum that is used for purposes such as collecting statistics, not for fine-grained synchronization control. Under low
 * update contention, the two classes have similar characteristics. But under high contention, expected throughput of
 * this class is significantly higher, at the expense of higher space consumption.
 *
...
final class LongAdder extends Striped64 implements Serializable, LongAddable {
    ...
    /**
     * Returns the current sum. The returned value is <em>NOT</em> an atomic snapshot; invocation in the absence of
     * concurrent updates returns an accurate result, but concurrent updates that occur while the sum is being
    ...
    /**
     * Equivalent in effect to {@link #sum} followed by {@link #reset}. This method may apply for example during
     * quiescent points between multithreaded computations. If there are updates concurrent with this method, the
     * returned value is <em>not</em> guaranteed to be the final value occurring before the reset.
     *
     * @return the sum
     */
