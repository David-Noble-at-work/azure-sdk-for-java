...
public abstract class LayoutType /*implements ILayoutType*/ {
     ...
     */
    protected LayoutType(@NotNull final LayoutCode code, final boolean immutable, final int size) {
     ...
     *
     * @return Success if the move is permitted, the error code otherwise. The source field is delete if the move
     * prepare fails with a destination error.
     */
    @NotNull
    ...
    public static Result prepareSparseRead(
        @NotNull final RowBuffer buffer, @NotNull final RowCursor edit, @NotNull LayoutCode code) {
    ...
    public static Result prepareSparseWrite(
        ...
        if (edit.scopeType().isFixedArity() && !(edit.scopeType() instanceof LayoutNullable)) {
            if ((edit.index() < edit.scopeTypeArgs().count())
                && !typeArg.equals(edit.scopeTypeArgs().get(edit.index()))) {
                return Result.TYPE_CONSTRAINT;
            }
