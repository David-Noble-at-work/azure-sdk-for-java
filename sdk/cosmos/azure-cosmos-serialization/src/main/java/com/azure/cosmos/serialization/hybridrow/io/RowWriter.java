...
import com.azure.cosmos.serialization.hybridrow.layouts.LayoutListWritable;
...
import com.azure.cosmos.serialization.hybridrow.layouts.LayoutUniqueScope;
import com.azure.cosmos.serialization.hybridrow.layouts.LayoutUtf8Writable;
...
public final class RowWriter {
     ...
     * <p>
     * A {@link RowWriter} instance writes the fields of a given scope from left to right in a forward only manner. If
     * the root scope is provided then all top-level fields in the row can be
     */
    ...
    public static <TContext> Result writeBuffer(
        @NotNull final RowBuffer buffer, final @NotNull TContext context, @NotNull final Writer<TContext> func) {
    ...
    public <TContext> Result writeScope(
        @NotNull final UtfAnyString path,
        @NotNull final TypeArgument typeArg,
    ...
    private Result prepareSparseWrite(UtfAnyString path, TypeArgument typeArg) {
        ...
        if (this.cursor.scopeType().isFixedArity() && !(this.cursor.scopeType() instanceof LayoutNullable)) {
            if ((this.cursor.index() < this.cursor.scopeTypeArgs().count())
                && !typeArg.equals(this.cursor.scopeTypeArgs().get(this.cursor.index()))) {
    ...
    private <TLayoutType extends LayoutType & LayoutListWritable<TValue>, TValue> Result writePrimitiveList(
        ...
        final TLayoutType type,
        final Consumer<List<TValue>> sparse) {
