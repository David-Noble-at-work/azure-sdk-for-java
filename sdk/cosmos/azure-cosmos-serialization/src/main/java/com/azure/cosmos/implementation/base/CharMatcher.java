...
import static com.azure.cosmos.implementation.base.Preconditions.checkPositionIndex;

/**
 * Determines a true or false value for any Java {@code char} value, just as {@link Predicate} does for any {@link
 * Object}*. Also offers basic text processing methods based on this function. Implementations are strongly encouraged
 * to be side-effect-free and immutable.
 *
 ...
 * points</a> in the range {@code 0x10000} to {@code 0x10FFFF} which includes the majority of assigned characters,
 * including important CJK characters and emoji.
 *
 * <p>Supplementary characters are <a
 * href="https://docs.oracle.com/javase/8/docs/api/java/lang/Character.html#supplementary">encoded into a {@code String}
 * using surrogate pairs</a>, and a {@code CharMatcher} treats these just as two separate characters. {@link #countIn}
 * counts each supplementary character as 2 {@code char}s.
 *
 * <p>For up-to-date Unicode character properties (digit, letter, etc.) and support for
 * supplementary code points, use ICU4J UCharacter and UnicodeSet (freeze() after building). For basic text processing
 * based on UnicodeSet use the ICU4J UnicodeSetSpanner.
 *
 * <p>Example usages:
 *
 * <pre>
...
public abstract class CharMatcher implements Predicate<Character> {
    ...
    /**
     * Constructor for use by subclasses. When subclassing, you may want to override {@code toString()} to provide a
     * useful description.
     */
    ...
    /**
     * Returns a {@code char} matcher that matches any BMP character present in the given character sequence. Returns a
     * bogus matcher if the sequence contains supplementary characters.
     *
    ...
    /**
     * Determines whether a character is a breaking whitespace (that is, a whitespace which can be interpreted as a
     * break between words for formatting purposes). See {@link #whitespace()} for a discussion of that term.
     *
     * @return the char matcher
     *
    ...
    /**
     * Returns a string copy of the input character sequence, with each group of consecutive matching BMP characters
     * replaced by a single replacement character. For example:
     *
     * <pre>{@code
    ...
    /**
     * Returns a matcher with identical behavior to the given {@link Character}-based predicate, but which operates on
     * primitive {@code char} instances instead.
     *
    ...
    /**
     * Returns a {@code char} matcher that matches any character in a given BMP range (both endpoints are inclusive).
     * For example, to match any lowercase letter of the English alphabet, use {@code CharMatcher.inRange('a', 'z')}.
     *
    ...
    /**
     * Returns the index of the first matching BMP character in a character sequence, or {@code -1} if no matching
     ...
     * {@code -1} if no character matches after that position.
     *
     ...
     *
     * @return the index of the first matching character, guaranteed to be no less than {@code start}, or {@code -1} if
    ...
    /**
     * Determines whether a character is a BMP digit according to {@linkplain Character#isDigit(char) Java's
    ...
    /**
     * Determines whether a character is a BMP letter according to {@linkplain Character#isLetter(char) Java's
     * definition}*. If you only care to match letters of the Latin alphabet, you can use {@code inRange('a',
     * 'z').or(inRange('A', 'Z'))}*.
     *
     * @return the char matcher
     *
    ...
    /**
     * Determines whether a character is a BMP letter or digit according to {@linkplain Character#isLetterOrDigit(char)
     * Java's definition}*.
     *
     * @return the char matcher
     *
    ...
    /**
     * Determines whether a BMP character is lower case according to {@linkplain Character#isLowerCase(char) Java's
     * definition}*.
     *
     * @return the char matcher
     *
    ...
    /**
     * Determines whether a BMP character is upper case according to {@linkplain Character#isUpperCase(char) Java's
     * definition}*.
     *
     * @return the char matcher
     *
    ...
    /**
     * Returns the index of the last matching BMP character in a character sequence, or {@code -1} if no matching
     * character is present.
     *
     ...
     *
     * @return {@code true} if this matcher matches every character in the sequence, including when the sequence is
     * empty
     */
    ...
    /**
     * Returns {@code true} if a character sequence contains at least one matching BMP character. Equivalent to {@code
    ...
    /**
     * Returns {@code true} if a character sequence contains no matching BMP characters. Equivalent to {@code
    ...
    /**
     * Returns a {@code char} matcher that matches any BMP character not present in the given character sequence.
    ...
    /**
     * Returns a {@code char} matcher functionally equivalent to this one, but which may be faster to query than the
     * original; your mileage may vary. Precomputation takes time and is likely to be worthwhile only if the precomputed
     * matcher is queried many thousands of times.
     *
     * <p>This method has no effect (returns {@code this}) when called in GWT: it's unclear whether a
     * precomputed matcher is faster, but it certainly consumes more memory, which doesn't seem like a worthwhile
     ...
     * @param replacement the character to append to the result string in place of each matching character in {@code
     * sequence}
     *
     * @return the new string
     */
    ...
    /**
     * Returns a string copy of the input character sequence, with each matching BMP character replaced by a given
     * replacement sequence. For example:
     *
     * <pre>{@code
     ...
     *
     * @param sequence the character sequence to replace matching characters in
     * @param replacement the characters to append to the result string in place of each matching character in {@code
     * sequence}
     *
     * @return the new string
     */
    ...
    /**
     * Determines whether a character is single-width (not double-width). When in doubt, this matcher errs on the side
     * of returning {@code false} (that is, it tends to assume a character is double-width).
     *
    ...
    /**
     * Returns a string representation of this {@code CharMatcher}, such as {@code CharMatcher.or(WHITESPACE,
     * JAVA_DIGIT)}.
     */
    @Override
    ...
    /**
     * Collapses groups of matching characters exactly as {@link #collapseFrom} does, except that groups of matching BMP
     * characters at the start or end of the sequence are removed without replacement.
     *
     * @param sequence the sequence
    ...
    /**
     * Returns a substring of the input character sequence that omits all matching BMP characters from the beginning and
    ...
    /**
     * Returns a substring of the input character sequence that omits all matching BMP characters from the beginning of
    ...
    /**
     * Returns a substring of the input character sequence that omits all matching BMP characters from the end of the
     * string. For example:
     *
     * <pre>{@code
    ...
    /**
     * Determines whether a character is whitespace according to the latest Unicode standard, as illustrated <a
     * href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=%5Cp%7Bwhitespace%7D">here</a>. This is not the same
     * definition used by other Java APIs. (See a <a href="https://goo.gl/Y6SLWx">comparison of several definitions of
     * "whitespace"</a>.)
     *
    ...
    /**
     * This is the actual implementation of {@link #precomputed}, but we bounce calls through a method on {@link
     * Platform}* so that we can have different behavior in GWT.
     *
     * <p>This implementation tries to be smart in a number of ways. It recognizes cases where the
     * negation is cheaper to precompute than the matcher itself; it tries to build small hash tables for matchers that
     * only match a few characters, and so on. In the worst-case scenario, it constructs an eight-kilobyte bit array and
     * queries that. In many situations this produces a matcher which is faster to query than the original.
     *
     * @return the char matcher
     */
    ...
    /**
     * Returns the Java Unicode escape sequence for the given {@code char}, in the form "\u12AB" where "12AB" is the
     * four hexadecimal digits representing the 16-bit code unit.
     */
    ...
    private static final class ForPredicate extends CharMatcher {
        ...
        /**
         * Instantiates a new For predicate.
         *
         * @param predicate the predicate
         */
