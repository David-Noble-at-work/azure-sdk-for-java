...
final class LittleEndianByteArray {
    ...
    /**
     * Load 8 bytes into long in a little endian manner, from the substring between position and position + 8. The array
     * must have at least 8 bytes from offset (inclusive).
     *
    ...
    /**
     * Similar to load64, but allows offset + 8 > input.length, padding the result with zeroes. This has to explicitly
     * reverse the order of the bytes as it packs them into the result which makes it slower than the native version.
     *
     * @param input the input bytes
    ...
    /**
     * Indicates that the loading of Unsafe was successful and the load and store operations will be very efficient. May
     * be useful for calling code to fall back on an alternative implementation that is slower than Unsafe.get/store but
     * faster than the pure-Java mask-and-shift.
     *
     * @return the boolean
     */
    ...
    /**
     * The only reference to Unsafe is in this nested class. We set things up so that if Unsafe.theUnsafe is
     * inaccessible, the attempt to load the nested class fails, and the outer class's static initializer can fall back
     * on a non-Unsafe version.
     */
