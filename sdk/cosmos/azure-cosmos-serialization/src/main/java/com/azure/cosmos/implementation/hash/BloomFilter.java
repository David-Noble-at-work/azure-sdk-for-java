...
import com.azure.cosmos.implementation.hash.BloomFilterStrategies.LockFreeBitArray;
 ...
 * href="http://llimllib.github.com/bloomfilter-tutorial/">tutorial</a> may help you understand how they work.
 *
 * <p>The false positive probability ({@code FPP}) of a Bloom filter is defined as the probability
 * that {@linkplain #mightContain(Object)} will erroneously return {@code true} for an object that has not actually been
 * put in the {@link BloomFilter Bloom filter}.
 *
 * <p>Bloom filters are serializable. They also support a more compact serial representation via the
 * {@link #writeTo} and {@link #readFrom} methods. Both serialized forms will continue to be supported by future
 * versions of this library. However, serial forms generated by newer versions of the code may not be readable by older
 * versions of the code (e.g., a serialized Bloom filter generated today may <i>not</i> be readable by a binary that was
...
public final class BloomFilter<T> implements Predicate<T> {
     ...
     * <p>
     * This method ensures that subsequent invocations of {@link #mightContain(Object)} with the same element will
    ...
    /**
     * Returns the probability that {@linkplain #mightContain(Object)} will erroneously return {@code true} for an
     * object that has not actually been put in the {@link BloomFilter Bloom filter}.
     *
     * <p>Ideally, this number should be close to the {@code fpp} parameter passed in {@linkplain
     * #create(Funnel, int, double)}*, or smaller. If it is significantly higher, it is usually the case that too many
     * elements (more than expected) have been put in the {@link BloomFilter Bloom filter}, degenerating it.
     *
    ...
    /**
     * Returns an estimate for the total number of distinct elements that have been added to this {@link BloomFilter
     * bloom filter}*.
     * <p>
     ...
     *
     * @return an estimate for the total number of distinct elements that have been added to this {@link BloomFilter
     * bloom filter}.
     *
     * @since 22.0
     */
    ...
    /**
     * Returns the number of bits in the underlying bit array.  @return the long
     */
    @TestOnly
    long bitSize() {
        return bits.bitSize();
    ...
    /**
     * Returns a {@code Collector} expecting the specified number of insertions, and yielding a {@link BloomFilter}*
     ...
     * resulting {@link BloomFilter Bloom filter} will suffer a sharp deterioration of its false positive probability.
     *
     * <p>The constructed {@link BloomFilter Bloom filter} will be serializable if the provided {@code Funnel<T>}
     * is.
     *
     * <p>It is recommended that the funnel be implemented as a Java enum. This has the benefit of
     * ensuring proper serialization and deserialization, which is important since {@link #equals} also relies on object
     * identity of funnels.
     *
     ...
     * <p>
     * Note that if the {@code Collector} receives significantly more elements than specified, the resulting {@link
     * BloomFilter Bloom filter} will suffer a sharp deterioration of its false positive probability.
     * <p>
     ...
     * filter}*.
     * <p>
     * The {@code Funnel} to be used is not encoded in the stream, so it must be provided here. <b>Warning:</b> the
     * funnel provided <b>must</b> behave identically to the one used to populate the original Bloom filter!
     *
