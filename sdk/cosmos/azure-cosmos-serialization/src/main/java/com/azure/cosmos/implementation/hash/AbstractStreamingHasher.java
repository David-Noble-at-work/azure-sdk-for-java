...
/**
 * A convenience base class for implementors of {@code Hasher}; handles accumulating data until an entire "chunk" (of
 * implementation-dependent length) is ready to be hashed.
 *
...
abstract class AbstractStreamingHasher extends AbstractHasher {
     ...
     *
     * @param chunkSize the number of bytes available per {@link #process(ByteBuffer)} invocation;     must be at least
     * 4
     */
    ...
    /**
     * Constructor for use by subclasses. This hasher instance will process chunks of the specified size, using an
     * internal buffer of {@code bufferSize} size, which must be a multiple of {@code chunkSize}*.
     *
     * @param chunkSize the number of bytes available per {@link #process(ByteBuffer)} invocation;     must be at least
     * 4
    ...
    /**
     * Processes the available bytes of the buffer (at most {@code chunk} bytes).  @param bb the bb
     */
    protected abstract void process(ByteBuffer bb);

    /**
     * This is invoked for the last bytes of the input, which are not enough to fill a whole chunk. The passed {@code
    ...
    /**
     * Computes a hash code based on the data that have been provided to this hasher. This is called after all chunks
     * are handled with {@link #process} and any leftover bytes that did not make a complete chunk are handled with
     * {@link #processRemaining}.
     *
