 ...
 * readers of the code and of stack traces. See
 * <a href="https://github.com/google/guava/wiki/ConditionalFailuresExplained">Conditional
 * failures explained</a> in the Guava User Guide for more advice. Notably, {@link Verify} offers assertions similar to
 * those in this class for non-precondition checks.
 *
 * <h3>{@code java.util.Objects.requireNonNull()}</h3>
 *
 * <p>Projects which use {@code com.google.common} should generally avoid the use of {@link
 * java.util.Objects#requireNonNull(Object)}. Instead, use whichever of {@link #checkNotNull(Object)} or {@link
 * Verify#verifyNotNull(Object)} is appropriate to the situation. (The same goes for the message-accepting overloads.)
 *
 * <h3>Only {@code %s} is supported</h3>
 *
 * <p>{@code Preconditions} uses {@link Strings#lenientFormat} to format error message template
 * strings. This only supports the {@code "%s"} specifier, not the full range of {@link java.util.Formatter} specifiers.
 * However, note that if the number of arguments does not match the number of occurrences of {@code "%s"} in the format
 * string, {@code Preconditions} will still behave as expected, and will still include all argument values in the error
 * message; the message will simply not be formatted exactly as intended.
 *
...
public final class Preconditions {
     ...
     * <p>
     * The message is formed by replacing each {@code %s} placeholder in the template with an argument. These are
     * matched by position - the first {@code %s} gets {@code errorMessageArgs[0]}, etc. Unmatched arguments will be
     * appended to the formatted message in square braces. Unmatched placeholders will be left as-is.
    ...
    /**
     * Ensures that {@code start} and {@code end} specify valid <i>positions</i> in an array, list or string of size
